...
export default function CategoryCard({ category }: { category: { name: string; count: number; icon: string } }) {
  const Icon = LucideIcons[category.icon as keyof typeof LucideIcons] as React.ElementType;
  return (
    <div className="group hover:border-primary hover:bg-primary border-border flex cursor-pointer flex-row items-center gap-8 border bg-white p-4 transition-all duration-300 ease-in-out sm:p-8 md:flex-col md:items-start">
      <div className="">
        {Icon && (
